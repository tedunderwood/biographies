...
# Make author centroids

auth_centroids = dict()

for author, docgroup in docsbyauthor.items():
    all_author_chars = []
    for doc in docgroup:
        if doc in vectorsbydoc:
            # metadata after all includes many docs not in this sample
            all_author_chars.extend(vectorsbydoc[doc])
    if len(all_author_chars) > 0:
        centroid = np.sum(all_author_chars, axis = 0) / len(all_author_chars)
        auth_centroids[author] = centroid
...
def adjusted_vector(charid):
    global chardict, meta, auth_centroids

    raw_vector = chardict[charid]

    docid = getdoc(charid)
    author = meta.loc[docid, 'author']
    normal_vector = auth_centroids[author]
