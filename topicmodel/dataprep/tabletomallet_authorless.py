...
with open('../dataprep/authorless.tsv', encoding = 'utf-8') as f:
    reader = csv.DictReader(f, delimiter = '\t')
    for row in reader:
        auth = row['author']
        if auth not in stopprobs:
            stopprobs[auth] = dict()
        word = row['word']
        stopprobs[auth][word] = float(row['stopprob'])

print('Stop probs read.')

sources = ['/Users/tunder/data/character_table_18c19c.tsv',
    '/Users/tunder/data/character_table_post1900.tsv']

malletout = '/Users/tunder/data/authorlessfic.txt'
...
def filter_words(original_string, lexicon, thefilter):
    ...
    with open(s, encoding = 'utf-8') as f:
        for line in f:
            ...
            if docid in randomsample or docid in specialids:
                ...
                if author in stopprobs:
                    authfilter = stopprobs[author]
                else:
                    authfilter = dict()
                charid = fields[2]
                date = fields[4]
                gender = fields[3]
                words = filter_words(fields[5], ficlexicon, authfilter)
