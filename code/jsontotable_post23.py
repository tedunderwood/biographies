...
def add_to_list(datalist, alist, prefix):
    global variants
...
with open('../metadata/hathi_ic_date.tsv', encoding = 'utf-8') as f:
    ...
    for row in reader:
        ...
        if row['authgender'] != '':
            authgen = row['authgender']
            id2authgen[docid] = authgen
            id2authgen[recordid] = authgen
...
def append_characters(jsonstring, outfile, expectedid):
    ...
    if storyid in id2date:
        ...
        docid = storyid
        authgender = id2authgen[storyid].lower()
    elif expectedid in id2date:
        date = id2date[expectedid]
        docid = expectedid
        authgender = id2authgen[expectedid].lower()
    else:
        unknowndate += 1
    ...
    for character in characterlist:
        ...
        thesewords = add_to_list(character["agent"], thesewords, '')
        thesewords = add_to_list(character["poss"], thesewords, '')
        thesewords = add_to_list(character["mod"], thesewords, '')
        thesewords = add_to_list(character["patient"], thesewords, 'was')

        for w in character["speaking"]:
            if w in stopwords:
                continue
        ...
        if len(thesewords) > 10:
            ...
            outline = '\t'.join([docid, namestring, docid + '|' + namestring, genderstring, authgender, str(date), textstring])
...
if os.path.isfile(outfile):
    ...
    with open(outfile, mode = 'w', encoding = 'utf-8') as f:
        f.write('docid\tcharname\tcharid\tchargender\tauthgender\tpubdate\twords\n')
